...
unsigned short prev_adc=0;
unsigned int flag=0;
unsigned char gear_index=0;
char gears[8][3]={"GN","G1","G2","G3","G4","G5","GR","-C"};
...
void main(void)
{
    ...
    while(1)
    {
        uint8_t speed_byte=read_speed();
        can_transmit(0x10,&speed_byte,1);
        for(int i=0;i<100;i++);

        uint8_t gear_byte=read_gear();
        can_transmit(0x20,&gear_byte,1);
        for(int i=0;i<100;i++);
    }
}
